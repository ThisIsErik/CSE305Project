...
void AntiDiagonalAux_MM_ScoreOnly(
    const std::string& A,
    const std::string& B,
    int mi, int ma, int g,
    int diagonal,
...
MMResultScore MyersMillerWavefront_ScoreOnly(
    const std::string& A,
    const std::string& B,
    int mi, int ma, int g,
    size_t num_threads
    ...
    for (int d = 1; d <= m + n; ++d) {
        ...
        if (len < static_cast<int>(num_threads * 100)) {
            LocalMax local;
            AntiDiagonalAux_MM_ScoreOnly(A, B, mi, ma, g, d, i_start, i_end,
        ...
        AntiDiagonalAux_MM_ScoreOnly(A, B, mi, ma, g, d, current_i, i_end,
                                     prev_diag, prev_prev_diag,
                                     wall_case,
